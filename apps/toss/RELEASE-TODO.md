# 픽플로우 앱인토스 — 남은 출시 작업 인계 문서

> 작성 2026-06-18. 코드/웹·API 배포는 완료(라이브). 아래는 **사람/토스 심사가 필요한 잔여 단계**예요.
> 관련: [README.md](./README.md), 콘솔 https://apps-in-toss.toss.im (workspace 51993 / mini-app 43785 / appName `pickflow`)
...
- 운영용 미니앱 번들 빌드: `apps/toss/pickflow.ait` (3.9MB, 운영 API origin 박힘)
...
1. **번들 업로드**: 콘솔 → `앱인토스` → `앱 출시` → 새 번들로 `apps/toss/pickflow.ait` 업로드
   - 번들 재생성이 필요하면(코드 변경 시):
     ```sh
     # 레포 루트에서
     VITE_API_BASE_URL=https://picky-olive.vercel.app pnpm build:toss
     # → apps/toss/pickflow.ait 갱신
     ```
2. **실기기 테스트(1회 이상 필수)**: 토스앱이 설치된 폰에서 테스트해야 `검토 요청` 버튼이 활성화돼요.
   - 샌드박스/QR로 `intoss://pickflow` 진입 → 둘러보기·작성·투표·공유 동작 확인
3. **검토 요청**: 콘솔에서 `검토 요청하기` → 토스 심사 **영업일 최대 3일**(비동기). 결과는 이메일 통보.
4. **출시**: 승인되면 콘솔 `출시하기` → 3000만 사용자에게 공개 (즉시 반영이므로 신중히)

> 검수 체크리스트(비게임)는 이미 충족: iframe 미사용 · 핀치줌 비활성 · TDS 사용 · 번들 ≤100MB(3.9MB) · `webViewProps: partner`. 콘솔 CORS는 API가 `origin: '*'`라 `pickflow.apps.tossmini.com`·`pickflow.private-apps.tossmini.com` 자동 허용.
