/**
 * 참여자 준비도 채점 — 웹(apps/web/ParticipantPreviewPanel)의 readiness 로직만 순수 함수로 떼어 이식.
...
const MAX_QUESTION_LEN = 90;
const MIN_OPTIONS = 2;
const MAX_OPTIONS = 6;
const MIN_CONTEXT_LEN = 20;

/** 질문/선택지/맥락 3항목을 채점한 준비도 결과. */
export const evaluatePollReadiness = (input: ReadinessInput): ReadinessResult => {
  ...
  if (normalizedQuestion.length < MIN_QUESTION_LEN) {
    questionHelp = '질문이 짧아 맥락을 이해하기 어려울 수 있어요.';
  } else if (normalizedQuestion.length > MAX_QUESTION_LEN) {
    questionHelp = '모바일에서 질문이 길게 느껴질 수 있어요.';
  } else {
    questionHelp = '첫 화면에서 바로 이해할 수 있는 길이예요.';
  }

  let optionHelp: string;
  if (hasDuplicateOptions) {
    optionHelp = '중복 선택지가 있어 참여자가 헷갈릴 수 있어요.';
  } else if (filledOptions.length > MAX_OPTIONS) {
    optionHelp = '선택지가 많아 즉시 응답에는 부담이 될 수 있어요.';
  } else if (filledOptions.length < MIN_OPTIONS) {
    optionHelp = '선택지를 2개 이상 채워야 투표를 만들 수 있어요.';
  } else {
    optionHelp = '선택지를 빠르게 비교할 수 있어요.';
  }

  const hasContext = normalizedDescription.length >= MIN_CONTEXT_LEN || attachmentCount > 0;
  const contextHelp = hasContext
    ? '배경 설명이 있어 판단 근거가 충분해요.'
    : '왜 투표하는지 한 줄 배경을 더하면 응답 품질이 좋아져요.';
